	...
	if ($elementLeaving.length && $elementEntering.length) {
		...
		$elementLeaving.fadeOut(300, function () {
			...
			callback1Promise.then(() => {
				...
				$elementEntering.css('display', 'none').css('visibility', 'inherit').fadeIn(300, function() {
...
function queueRabbitAnimation (numberRabbits, startRow) {
	...
	function dropRabbit() {
		...
		if (rabbitRowHeight > 0) {
			// For now, reducing the rabbitRowHeight by 1 just so we don't have a mountain stacked on one rabbit, but let's add better collision-like logic later - maybe storing a top layer x position object somewhere
